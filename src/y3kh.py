...
def trackerDetection(tracker,image,centers,number,max_point_distance = 30,max_colors = 20,track_id_size = 0.8):
    ...
    cv2.putText(result, str(number), (20,  40), font, 1, (0, 0, 255), 2)  # 左上角，人数计数
...
if __name__ == '__main__':
    ...
    inputPath = 'basketball/'
    outputPath = 'basketball/'
    ...
    mkdir('result/' + outputPath)
    for n in tqdm(range(100)):
        image = Image.open('dataset/' + inputPath + '%s.jpg'%('%s'%(n + 1)).zfill(4))
        ...
        misc.imsave('result/' + outputPath + '%s.jpg'%n, result)
    ...
    #     cv2.imwrite('result/%s.jpg'%n,result, [int(cv2.IMWRITE_JPEG_QUALITY), 100] )
    #     n += 1
    #     end = timer()
    ...
    # 图片变视频
    output_dir='result/' + outputPath
    output_path= os.path.join(output_dir, '')                       # 输入图片存放位置
    output_video_file = 'result/' + outputPath + 'result.mp4' # 输入视频保存位置以及视频名称
